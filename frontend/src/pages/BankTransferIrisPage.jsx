...
const IRIS_HINT =
  import.meta.env.VITE_IRIS_HINT ||
  "Χρησιμοποιήστε την εφαρμογή της τράπεζάς σας για πληρωμή μέσω IRIS.";
...
export default function BankTransferIrisPage() {
  ...
  const handleFinish = () => {
    alert("Σας ευχαριστούμε πολύ για την ολοκλήρωση της συναλλαγής.");
    clearCart();
    navigate("/");
  };
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <header className="space-y-1">
        ...
        <p className="text-sm text-slate-600">
          Ολοκληρώσατε την παραγγελία σας. Για να ολοκληρωθεί η πληρωμή, παρακαλούμε πραγματοποιήστε την
          πληρωμή μέσω τραπεζικής κατάθεσης ή IRIS, χρησιμοποιώντας τα παρακάτω στοιχεία.
        </p>
      </header>

      {/* Order summary */}
      <section className="rounded-xl border bg-white p-4 space-y-2">
        <h2 className="text-sm font-semibold text-slate-800">Σύνοψη παραγγελίας</h2>
        <div className="text-sm text-slate-700">
          <div>
            <span className="font-medium">Αριθμός παραγγελίας:</span> <span>{orderId}</span>
          </div>
          <div>
          ...
          <div className="text-xs text-slate-500 mt-1">
            * Παρακαλούμε συμπληρώστε τον αριθμό παραγγελίας ή τον κωδικό πληρωμής ως αιτιολογία.
          </div>
        </div>
      </section>
      ...
      <section className="rounded-xl border border-sky-100 bg-sky-50 p-4 space-y-3">
        <div className="flex items-center justify-between gap-4">
          <div>
            <h2 className="text-sm font-semibold text-sky-900">Πληρωμή μέσω IRIS</h2>
            <p className="text-xs text-sky-900">
              Ανοίξτε την εφαρμογή της τράπεζάς σας και επιλέξτε πληρωμή μέσω IRIS. Σκανάρετε το QR ή
              χρησιμοποιήστε τα στοιχεία.
            </p>
          </div>
          <div
            className="flex h-24 w-24 items-center justify-center rounded-lg border border-sky-200 bg-white cursor-pointer"
            onClick={() => setIsQRModalOpen(true)}
          >
            <img src={irisQR} alt="IRIS QR" className="h-full w-full object-contain rounded-lg" />
          </div>
        </div>
      ...
      <section className="rounded-xl border bg-white p-4 space-y-3">
        <h2 className="text-sm font-semibold text-slate-800">Στοιχεία Τραπεζικής Κατάθεσης</h2>
        <div className="space-y-2">
          ...
          <CopyField label="Αιτιολογία (Πολλύ Σημαντικό)" value={paymentReference} />
        </div>
        <p className="text-xs text-slate-600 mt-2">
          Μόλις πραγματοποιηθεί η κατάθεση, η παραγγελία σας θα επιβεβαιωθεί μέσα σε 1–2 εργάσιμες
          ημέρες. Μπορείτε να μας στείλετε και το αποδεικτικό στο{" "}
          <a href="mailto:info@lookoptica.gr" className="underline text-amber-700">
            info@lookoptica.gr
          </a>{" "}
      ...
      {isQRModalOpen && (
        ...
        >
          <div
            className="bg-white p-4 rounded-xl shadow-xl max-w-sm w-full"
            onClick={(e) => e.stopPropagation()}
          >
            <img src={irisQR} alt="IRIS QR Fullscreen" className="w-full h-auto rounded-lg" />

            <button
              type="button"
              className="mt-4 w-full rounded-lg bg-amber-700 py-2 text-sm font-semibold text-white hover:bg-amber-800"
              onClick={() => setIsQRModalOpen(false)}
            >
              Κλείσιμο
            </button>
