...
function mapProductForTable(product) {

  return {

    ...product,

    categoryNormalized: resolveCategoryValue(product.category),

    categoryLabel: categoryLabelFromValue(product.category),

    _mainImage: Array.isArray(product.images) && product.images.length > 0 ? product.images[0] : "",

    _editPrice: product.discountPrice ?? "",

    _editDiscountPrice: product.price ?? "",

    _editStatus: product.status || "in_stock",

    _editBrand: product.brand || "",

    _editCategory: resolveCategoryValue(product.category) || product.category || "",

    _editStock: product.stock ?? 0,
...
export default function AdminProductsPage() {
  ...
  const [pageSize, setPageSize] = useState(() => {
    ...
    return PAGE_SIZE_OPTIONS.includes(param) ? param : PAGE_SIZE_OPTIONS[0];
  });
  ...
  useEffect(() => {
    ...
    const normalizedPerPage = PAGE_SIZE_OPTIONS.includes(rawPerPage)
      ? rawPerPage
      : PAGE_SIZE_OPTIONS[0];
    if (normalizedPerPage !== pageSize) {
      setPageSize(normalizedPerPage);
    }
  ...
  useEffect(() => {
    ...
    async function loadAllIncrementally() {
      ...
      try {
        ...
        while (true) {
          ...
          const enriched = list.map(mapProductForTable);
          ...
          if (isFirstBatch) {
            ...
            setAllItems((prev) => [...prev, ...enriched]);
          ...
          enriched.forEach((p) => {
            ...
            if (catVal) categorySet.add(catVal);

          });



          if (!cancelled && brandSet.size > 0) {

            const nextBrands = Array.from(brandSet).sort((a, b) => a.localeCompare(b));
          ...
          if (!cancelled && categorySet.size > 0) {

            const nextCategories = Array.from(categorySet)
        ...
        if (!cancelled) {
          ...
          setErrorMsg("ƒœ¤ ã«˜¤ ›¬¤˜«ã ž ­æ¨«à©ž «à¤ §¨¦äæ¤«à¤.");
          setState("error");
  ...
  function handleEdit(product) {
    ...
    navigate({
      pathname: `/admin/products/${product.slug}/edit`,
      search: query ? `?${query}` : "",
    });
  }
